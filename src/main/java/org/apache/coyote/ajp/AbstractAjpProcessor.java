...
public abstract class AbstractAjpProcessor<S> extends AbstractProcessor<S> {
	...
	protected abstract int output(byte[] src, int offset, int length, boolean block) throws IOException;

	// Methods called by process()
	protected abstract void setupSocket(SocketWrapper<S> socketWrapper) throws IOException;

	protected abstract void setTimeout(SocketWrapper<S> socketWrapper, int timeout) throws IOException;
	 ...
	 * @throws IOException
	 */
	protected abstract boolean read(byte[] buf, int pos, int n, boolean block) throws IOException;
	...
	private void writeResponseMessage(boolean block) throws IOException {
		...
		while (written > 0 && responseMsgPos < len) {
			written = output(responseMessage.getBuffer(), responseMsgPos, len - responseMsgPos, block);
